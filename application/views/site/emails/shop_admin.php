<div style="border: 1px solid #d8d8d8;">
	...
	<div style="padding: 20px 20px 40px; font-size: 14px; line-height: 21px;">
		Новый заказ <strong>#<?=$alias;?></strong><br/>
		<br/>
		<strong>Имя</strong>: <?=$name;?><br/>
		<strong>Дата</strong>: <?=date('d.m.Y H:i', strtotime($date));?><br/>
		<strong>Телефон</strong>: <?=$phone;?><br/>
		<strong>E-mail</strong>: <?=$email;?><br/>
		
		<? if(isset($adres)) { ?><strong>Адрес</strong>: <?=$adres;?><br/><? } ?>
		<? if(isset($text)) { ?><strong>Комментарий</strong>: <?=$text;?><br/><? } ?>
		...
		<?=anchor('/admin/orders/view/'.$alias, '', array('style' => 'color: #f02c3e'));?>
	</div>
</div>