<? function catalog_option($_item) { ?>
	<? $_count_nbs = ($_item['level'] - 1) * 3; ?>
	<option value="<?=$_item['idItem']?>" <?=set_select('idParent', $_item['idItem']);?>>
		<?=nbs($_count_nbs);?>
		<?/* for($i = 1; $i < $item['level']; $i++) { ?>---<? } */?>
		<?=$_item['title'];?>
	</option>
	<? if(isset($_item['childs'])) { ?>
		<? foreach($_item['childs'] as $childs) { ?>
...
<div class="row form-group">
	...
	<div class="col-9 form-colinput">
		<input type="text" class="form-input" name="title" data-toggle="translate_auto" value="<?=set_value('title');?>" />
		<?=form_error('title'); ?>
	</div>
</div>
<div class="row form-group">
...
<div class="row form-group">
	<div class="col-3 form-collabel">
		Основная категория <span class="required">*</span>
	</div>
	<div class="col-9 form-colinput">
