<? function catalog_option($item, $current) { ?>
	<? $_count_nbs = ($item['level'] - 1) * 3; ?>
	<option value="<?=$item['idItem']?>" <?=set_select('idParent', $item['idItem'], $item['idItem'] == $current ? true : false);?>>
		<?=nbs($_count_nbs);?>
...
<div class="row form-group">
	...
	<div class="col-9 form-colinput">
		...
		<? foreach($parents as $parent) { ?>
			<?=catalog_option($parent, $item['idParent']);?>
		<? } ?>
		</select>
		<?=form_error('idParent'); ?>
	</div>
