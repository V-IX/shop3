...
class Unit_currency extends ADMIN_Controller {
	
	protected $model = '';
	protected $page = 'unit_currency';
	...
	public function index()
	{
		...
		$count = $this->model->countCurrency();
		$pagination = admin_pagination(uri(2).'/index', $count);
		
		$data['items'] = $this->model->itemsCurrency($pagination['per_page'], $pagination['offset'], 'value|DESC');
	...
	public function create()
	{
		...
		$error = false;
		if($this->input->post() and $this->model->validate('currency'))
		{
			$error = $this->model->insertCurrency();
	...
	public function edit()
	{
		...
		if($this->input->post() and $this->model->validate('currency'))
		{
			$error = $this->model->updateCurrency(uri(4));
			if(!$error)
			{
		...
		$data['item'] = $this->model->itemCurrency(uri(4));
		if(empty($data['item']))
		{
	...
	{
		if($this->input->post()) $this->model->deleteCurrency(uri(4));
		redirect('admin/'.uri(2));
	}
}
